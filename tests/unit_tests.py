...
def test_Upload_Time_series():
    ...
    sqldatabase.Load_Time_Series_Global_Confirmed_Data(os.path.join(dirname, 'staticTestFile/time_series_covid19_confirmed_global.csv'))
    ...
    sqldatabase.Load_Time_Series_Global_Deaths_Data(os.path.join(dirname, 'staticTestFile/time_series_covid19_deaths_global.csv'))
    time1 = datetime.datetime(2020, 6, 18, 0, 0)
    time2 = datetime.datetime(2020, 6, 18, 0, 0)
    data = sqldatabase.query_by_combined_key(time1, time2, 'Alberta,Canada')
    ...
    # 3 new rows should be added in the queried result, as ",Canada" indicates a new key
    sqldatabase.Load_Time_Series_Global_Recovered_Data(os.path.join(dirname, 'staticTestFile/time_series_covid19_recovered_global.csv'))
    time1 = datetime.datetime(2020, 6, 18, 0, 0)
    time2 = datetime.datetime(2020, 6, 18, 0, 0)
    data = sqldatabase.query_by_combined_key(time1, time2, 'Canada')
    assert data[0] == (None, None, 63782.0, None, datetime.datetime(2020, 6, 18, 0, 0), None, 'Canada', None)
    time1 = datetime.datetime(2020, 6, 17, 0, 0)
    time2 = datetime.datetime(2020, 6, 17, 0, 0)
    data = sqldatabase.query_by_combined_key(time1, time2, 'Afghanistan')
    assert data[0] == (491.0, 26874.0, 6158.0, None, datetime.datetime(2020, 6, 17, 0, 0), None, 'Afghanistan', None)
    data = sqldatabase.query_all()
    assert len(data) == 9
